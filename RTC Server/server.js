...
function handle_socket(socket) {
  ...
  socket.on('login', function(data) {
    ...
    log('before login, email: ' + email + ', pwd: ' + pwd);
  ...
  socket.on('offer', function(sdp) {
    log('offer ready, broadcaster: ' + socket.email);

    if(!socket.email) { // Only broadcaster
      return;
    }

    socket.broadcast.to(socket.email).emit('offer', sdp);
  ...
  socket.on('answer', function(sdp) {
    ...
    socket.broadcast.to(socekt.room).emit('answer', sdp);

    log('answer, viewer: ' + socket.room);
  });
